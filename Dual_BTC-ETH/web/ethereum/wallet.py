...
def validate_rpc_url(rpc_url: str) -> str:
    ...
    if not value or len(value) > 2048:
        raise WalletError("JSON-RPC URL is empty or too long")
    try:
        parsed = urlsplit(value)
        port = parsed.port
    except ValueError as exc:
        raise WalletError("Invalid JSON-RPC URL") from exc
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise WalletError("JSON-RPC URL must use http:// or https:// and include a host")
    if parsed.username is not None or parsed.password is not None:
        raise WalletError(
            "Do not put RPC credentials in the URL; use a trusted proxy or API-key path"
        )
    if parsed.fragment:
        raise WalletError("JSON-RPC URL must not contain a fragment")
    if port is not None and not 1 <= port <= 65535:
        raise WalletError("JSON-RPC port must be between 1 and 65535")
    return value


def connect(rpc_url: str) -> Web3:
    validated_url = validate_rpc_url(rpc_url)
    web3 = Web3(Web3.HTTPProvider(validated_url, request_kwargs={"timeout": 15}))
    if not web3.is_connected():
        raise WalletError(f"Cannot connect to JSON-RPC: {validated_url}")
    return web3
