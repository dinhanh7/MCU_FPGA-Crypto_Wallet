...
const API_PREFIX = window.location.pathname.startsWith("/eth") ? "/eth" : "";
...
function rpcUrl() {
  ...
  try {
    ...
    throw new Error("URL Anvil không hợp lệ");
  }
  if (!["http:", "https:"].includes(parsed.protocol) || !parsed.hostname) {
    throw new Error("URL Anvil phải dùng http:// hoặc https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("Không đặt username/password trực tiếp trong URL Anvil");
  }
  return value;
}
...
function updateWalletSummary() {
  ...
  $("signerMode").textContent = fpgaSelected
    ? `Dual signer · MCU review/freeze/PIN · Gowin ACG525 ký và tự kiểm tra`
    : cSelected
...
async function signUnsigned() {
  ...
  if (!cSelected && !password) {
    toast(fpgaSelected ? "Nhập mã mở khóa MCU" : "Nhập mật khẩu keystore", true);
    return;
  }
  ...
  try {
    const result = fpgaSelected
      ? await api("/api/sign-fpga", { unsigned: state.unsigned, passkey: password })
      : cSelected
    ...
    const signedMessage = fpgaSelected
      ? "MCU đã review/freeze; FPGA ký qua UART; MCU recover kiểm tra chữ ký"
      : cSelected
