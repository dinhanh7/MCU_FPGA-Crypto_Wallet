...
module tb_btc_fpga_signer_core;
  ...
  btc_fpga_signer_core #(.PRIVATE_KEY(256'h1)) dut (
    ...
    .buffer_read_byte(),
    .start(start),.direct_hash_mode(1'b0),
    .direct_hash_load_valid(1'b0),.direct_hash_load_byte(8'b0),
    .tx_version(32'h02000000),
    ...
    .bip143_digest(digest),.y_parity(),.recovery_high(),
    .signature_r(r),.signature_s(s)
  );
