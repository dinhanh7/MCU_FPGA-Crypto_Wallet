...
// HMAC-SHA256 byte source specialized for the three RFC6979 messages:
//   V, V||marker, and V||marker||private_key||reduced_hash.
// The caller holds all inputs stable until done, so no 776-bit message latch
// or variable byte-selection mux is required.
module rfc6979_hmac_sha256 (
  input  logic         clk,
  input  logic         reset_n,
  input  logic         start,
  ...
  input  logic [1:0]   message_kind, // 0: V, 1: V||marker, 2: V||marker||x||h
  output logic         busy,
  output logic         done,
  ...
  logic sha_start,sha_in_valid,sha_in_ready,sha_in_last,sha_done;
