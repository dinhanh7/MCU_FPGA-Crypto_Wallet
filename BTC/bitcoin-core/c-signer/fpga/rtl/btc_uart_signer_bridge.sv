...
module btc_uart_signer_bridge #(
  ...
  localparam logic [7:0] VERSION=8'h01,COMMAND_PING=8'h00,COMMAND_SIGN=8'h10;
  ...
  logic [31:0] response_word_shift,response_next_word;
  logic [5:0] response_next_word_index;
  ...
  always_comb begin
    request_buffer_read_enable=response_active &&
      response_command==COMMAND_SIGN && response_status==STATUS_OK;
    ...
    response_next_word_index=((response_index-9'd40)>>2)+6'd1;
    response_next_word=32'b0;
    case(response_next_word_index)
       1:response_next_word=signer_digest[223:192];
       2:response_next_word=signer_digest[191:160];
       3:response_next_word=signer_digest[159:128];
       4:response_next_word=signer_digest[127:96];
       5:response_next_word=signer_digest[95:64];
       6:response_next_word=signer_digest[63:32];
       7:response_next_word=signer_digest[31:0];
       8:response_next_word=signer_r[255:224];
       9:response_next_word=signer_r[223:192];
      10:response_next_word=signer_r[191:160];
      11:response_next_word=signer_r[159:128];
      12:response_next_word=signer_r[127:96];
      13:response_next_word=signer_r[95:64];
      14:response_next_word=signer_r[63:32];
      15:response_next_word=signer_r[31:0];
      16:response_next_word=signer_s[255:224];
      17:response_next_word=signer_s[223:192];
      18:response_next_word=signer_s[191:160];
      19:response_next_word=signer_s[159:128];
      20:response_next_word=signer_s[127:96];
      21:response_next_word=signer_s[95:64];
      22:response_next_word=signer_s[63:32];
      23:response_next_word=signer_s[31:0];
      default:response_next_word=32'b0;
    endcase
  ...
  always_ff @(posedge clk or negedge reset_n) begin
    if(!reset_n) begin
      ...
      if(response_active && !tx_busy && !tx_start) begin
        ...
        if(response_command==COMMAND_SIGN &&
           response_index>=40 && response_index<8+response_payload_length) begin
          if(response_index[1:0]==2'b11 && response_index<9'd135)
            response_word_shift<=response_next_word;
        ...
        case(parser_state)
          ...
          P_LEN_LO:begin
            ...
            if({request_len_hi,rx_data}>MAX_PAYLOAD ||
               (request_command==COMMAND_SIGN && {request_len_hi,rx_data}<114)) begin
          ...
          P_CRC_LO:begin
            ...
            end else if(request_command!=COMMAND_SIGN) begin
              if(!response_active)begin_response(request_sequence,request_command,STATUS_COMMAND,0);
      ...
      if(sign_result_pending && !response_active) begin
        ...
        if(signer_error) begin
          begin_response(sign_sequence,COMMAND_SIGN,
            signer_error_code==1?STATUS_KEY:
            signer_error_code==2?STATUS_BIP143:STATUS_VERIFY,0);
          last_success<=0;
        end else begin
          response_word_shift<=signer_digest[255:224];
          begin_response(sign_sequence,COMMAND_SIGN,STATUS_OK,16'd128);
          last_success<=1;
        end
      end
    end
