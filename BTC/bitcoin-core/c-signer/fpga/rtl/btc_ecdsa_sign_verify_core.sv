...
`include "eth_secp256k1_common.svh"
  ...
  btc_rfc6979_nonce nonce_inst (
    ...
    // btc_fpga_signer_core keeps the completed BIP143 digest stable for the
    // whole signing operation; use it directly instead of latching 256 bits.
  ...
  always_ff @(posedge clk or negedge reset_n) begin
    if(!reset_n) begin
      ...
      verified<=0;signature_r<=0;signature_s<=0;
    end else begin
      done<=1'b0;
      case(state)
        S_IDLE:if(start) begin
          error<=0;error_code<=0;verified<=0;signature_r<=0;signature_s<=0;
