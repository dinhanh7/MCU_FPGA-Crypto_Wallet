...
PIN = "246810"
PREFIX = "/tmp/mcu-manual-qr-e2e-20260721"
...
assert "onclick=nextQr()" in large_html_text
assert "setInterval" not in large_html_text

print(json.dumps({
    "transport":"manual-step BBQr Z/P, max 100 bytes per QR",
    ...
    "manual_next_button": "onclick=nextQr()" in large_html_text,
    "automatic_qr_timer_absent": "setInterval" not in large_html_text,
